...
/*   Updated: 2025/03/11 11:52:04 by jrollon-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	update_frame(t_window *win)
{
	...
	{
		if (x > WIDTH - win->sprite.width)
			x = WIDTH - win->sprite.width;
		x++;
	}
	...
	{
		if (y > HEIGHT - win->sprite.height)
			y = HEIGHT - win->sprite.height;
		y++;
	}
	...
	mlx_put_image_to_window(win->mlx, win->win, win->sprite.img.img, x, y);
	return (0);
}
